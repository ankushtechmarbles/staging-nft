var Paint = (function () {
  ...
  var PANEL_WIDTH = 0; // 300
  var PANEL_HEIGHT = 0; // 580
  ...
  var ROUGHNESS = 0.075;
  var F0 = 0.05;
  // var SPECULAR_SCALE = 0.5; // 0.5
  // var DIFFUSE_SCALE = 0.15; // 0.15
  ...
  function makeBlurShader(width) {
    ...
    for (var i = 0; i < width; ++i) {
      ...
      shader +=
        "total += texture2D(u_input, coordinates + delta * " +
        offset.toFixed(1) +
        ") * " +
    ...
    shader +=
      "gl_FragColor = total / " + normalizationFactor.toFixed(1) + ";\n }";

    return shader;
  }

  // ! Remove: No longer needed, everything has been set to RGB
  ...
  function Paint(
    ...
    this.setCanvasDataUrl = setCanvasDataUrl;
    this.initColor = initColor;
    this.initSize = initSize;
    this.SPECULAR_SCALE = specular_scale;
    this.DIFFUSE_SCALE = diffuse_scale;
    ...
    function start(shaderSources) {
      ...
      for (var i = 0; i < HISTORY_SIZE; ++i) {
        //we always keep around HISTORY_SIZE snapshots to avoid reallocating textures
        var texture = wgl.buildTexture(
          wgl.RGBA,
          wgl.FLOAT,
          this.getPaintingResolutionWidth(),
          this.getPaintingResolutionHeight(),
      ...
      this.brushScale = this.initSize;

      this.brushColorHSVA = this.initColor;
      ...
      // ! OnResize controls the size of the canvas.
      ...
      // This is what keeps the canvas running
      var update = async function () {
        this.needsRedraw = true;
        this.update();
        this.wgl.gl.clear(this.wgl.gl.DEPTH_BUFFER_BIT);
        requestAnimationFrame(update);

        // if (!this.colorPicker.display) {
        //   const dataUrl = this.wgl.gl.canvas.toDataURL();
        //   this.setCanvasDataUrl(dataUrl);
        // }
      }.bind(this);
      update();
    }
  ...
  Paint.prototype.update = function () {
    ...
    if (this.needsRedraw) {
      ...
      var paintingDrawState = wgl
        ...
        .uniform1f("u_diffuseScale", this.DIFFUSE_SCALE)
        .uniform1f("u_specularScale", this.SPECULAR_SCALE)
        .uniform1f("u_F0", F0)
        .uniform3f(
          "u_lightDirection",
          LIGHT_DIRECTION[0],
    ...
    if (
      ...
      (!this?.colorPicker?.isInUse() &&
        this.interactionState === InteractionMode.NONE &&
        this.desiredInteractionMode(this.mouseX, this.mouseY) ===
          InteractionMode.PAINTING)
    ) {
      ...
      var brushDrawState = wgl
        ...
        .uniformTexture(
          ...
          0,
          wgl.TEXTURE_2D,
          this.brush.positionsTexture
        );

      wgl.drawElements(
        brushDrawState,
        wgl.LINES,
        (this.brush.indexCount * this.brush.bristleCount) /
          this.brush.maxBristleCount,
        wgl.UNSIGNED_SHORT,
        0
      );
    }
    ...
    if (this?.colorPicker?.isInUse()) {
      ...
      if (desiredMode === InteractionMode.PAINTING) {
        desiredCursor = "none";
        // ! Remove: Ability to resize the painting by clicking the edges
      } else if (desiredMode === InteractionMode.RESIZING) {
        // desiredCursor = cursorForResizingSide(
        //   this.getResizingSide(this.mouseX, this.mouseY)
        // );
  ...
  Paint.prototype.applySnapshot = function (snapshot) {
    ...
    if (
      this.simulator.width !== this.getPaintingResolutionWidth() ||
      this.simulator.height !== this.getPaintingResolutionHeight()
    ) {
      this.simulator.changeResolution(
        this.getPaintingResolutionWidth(),
        this.getPaintingResolutionHeight()
  ...
  //! Moved these to useEffect in painterGame
  ...
  // ! All references to refresh needs to be removed
  ...
  Paint.prototype.save = function () {
    ...
    var saveTexture = wgl.buildTexture(
      wgl.RGBA,
      wgl.UNSIGNED_BYTE,
      saveWidth,
      saveHeight,
    ...
    var paintingProgram =
      this.colorModel === ColorModel.RYB
        ? this.savePaintingProgram
        : this.savePaintingProgramRGB;

    var saveDrawState = wgl
      .createDrawState()
  ...
  Paint.prototype.onMouseMove = function (event) {
    ...
    // ! Removed edge detection, commented out code
  ...
  Paint.prototype.getResizingSide = function (mouseX, mouseY) {
    ...
    if (
      mouseY > this.paintingRectangle.bottom &&
      mouseY <= this.paintingRectangle.getTop()
    ) {
      //left or right
    ...
    if (
      mouseX > this.paintingRectangle.left &&
      mouseX <= this.paintingRectangle.getRight()
    ) {
      //bottom or top
  ...
  Paint.prototype.desiredInteractionMode = function (mouseX, mouseY) {
    var mouseOverPanel =
      mouseX < PANEL_WIDTH && mouseY > this.canvas.height - PANEL_HEIGHT;

    if (mouseOverPanel) {
      return InteractionMode.NONE;
      // !Edge detection if/else conditions did go here
    } else {
  ...
  Paint.prototype.onMouseDown = function (event) {
    ...
    if (!this?.colorPicker?.isInUse()) {
      ...
      if (mode === InteractionMode.PANNING) {
        ...
        if (typeof wgl === "undefined" || this.wgl === undefined) {
          this.saveSnapshot();
        }
      }
    }
  ...
  Paint.prototype.onTouchStart = function (event) {
    ...
    // ! Remove: panning mode for mobile interaction
